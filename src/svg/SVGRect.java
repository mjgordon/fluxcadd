...
public class SVGRect extends SVGElement {
	...
	public void bake(GeometryFile geom, float hatchWidth) {
		...
		if (stroked) geom.add(l1);
		
		l2 = new Line(new PVector(x+width,y),new PVector(x+width,y+height));
		l2.color(strokeColor);
		if (stroked) geom.add(l2);
		
		l3 = new Line(new PVector(x+width,y+height),new PVector(x,y+height));
		l3.color(strokeColor);
		if (stroked) geom.add(l3);
		
		l4 = new Line(new PVector(x,y+height),new PVector(x,y));
		l4.color(strokeColor);
		if (stroked) geom.add(l4);
	...
	public void plot(ArrayList<String> out, float hatchWidth) {
		...
		out.add("LIN {X " + l1.startPoint.x * s + " ,Y " + l1.startPoint.y * s + " ,Z -10}");
		out.add("LIN {Z 0}");
